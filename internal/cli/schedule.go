...
var tripSetTime = wrap(func(args []string) error {
	...
	// A place can legitimately appear twice in one day — a hotel at bag-drop and
	// again at check-in — so the occurrence has to be addressable.
	...
	if *nth < 1 || *nth > len(matches) {
		return fmt.Errorf("--nth %d is out of range: %q appears %d time(s) in section %s",
